...
OFFICIAL_REL = Path("paper_ready/phase9a_malcl_reproduction_design/external_code/MalCL_official")
...
Important documented adaptation: the official discriminator flattened FC head is replaced by a T4-memory-safe pooled discriminator head. This avoids an impractically large `256 x feature_dim -> 1024` layer during full Colab runs. Therefore, this output should be framed as a protocol-matched, T4-memory-safe MalCL baseline candidate, not an exact official MalCL reproduction.
...
def main() -> int:
    ...
    parser.add_argument("--drive-output-zip", default="/content/drive/MyDrive/AHR_MalCL/final_malcl_baseline_results_colab.zip")
