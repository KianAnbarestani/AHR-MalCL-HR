...
print("discriminator_topology: feature-dimension-parameterized flattened discriminator")
print("baseline_label: protocol-matched adapted MalCL baseline")
...
PROJECT_ZIP = Path(os.environ["AHR_PROJECT_ZIP"])
DRIVE_DATA_DIR = Path(os.environ["AHR_DATA_SOURCE_DIR"])
CONTENT_DATA_DIR = Path(os.environ["AHR_RUNTIME_DATA_DIR"])
WORKDIR = Path(os.environ["AHR_WORKDIR"])
DRIVE_OUTPUT_ZIP = Path(os.environ["AHR_OUTPUT_ZIP"])
DRIVE_MOUNT = os.environ["AHR_DRIVE_MOUNT"]
...
def unzip_project():
    ...
    with zipfile.ZipFile(PROJECT_ZIP, "r") as zf:
        zf.extractall(WORKDIR.parent)
    if not WORKDIR.exists():
        candidates = [p for p in WORKDIR.parent.glob("**/run_final_malcl_baseline.py") if "final_malcl_baseline_colab_run" in str(p)]
        if not candidates:
